  ...
  return (
    <div className="bg-white rounded-lg p-6">
      <p className="text-xs text-gray-700 leading-relaxed">
        ...
        provides accurate traceability from origin to end user. Adhering to best
        practices in product tracking, quality control, and timely updates is
        essential to uphold customer trust and operational excellence. This
        guide aims to support a streamlined process, reduce inefficiencies, and
        promote accountability throughout the product's lifecycle.
      </p>
      ...
      <div className="grid py-6 space-y-3 bg-white rounded-lg divide-y divide-gray-100 text-sm text-gray-800">
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p className="text-gray-700">{productDetailData.name}</p>
        </div>
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p className="text-gray-700">{productDetailData.origin}</p>
        </div>
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p className="text-gray-700">
            {productDetailData.currentOwner.slice(0, 20)}...
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p
            className={`inline-flex items-center gap-x-2 text-xs rounded-lg bg-indigo-200 px-2 py-0.5 border border-transparent  focus:outline-none  disabled:opacity-50 disabled:pointer-events-none uppercase ${
              productDetailData.status === 1
                ? "text-indigo-700 bg-indigo-200"
                : "bg-green-200 text-green-700"
            }`}
          >
            {productDetailData.status === 1 ? "In Transit" : "Delivered"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p className="text-gray-700">
            {new Date(productDetailData.timestamp).toLocaleString()}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p className="text-gray-700">{productDetailData.location}</p>
        </div>
        ...
        <div className="flex items-center gap-4 py-3">
          ...
          <p className="text-gray-700">{productDetailData.remarks}</p>
        </div>
      </div>
    </div>
  );
